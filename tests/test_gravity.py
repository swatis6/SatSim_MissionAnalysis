...
def test_two_body_acceleration_magnitude():
    ...
    assert abs(np.linalg.norm(a) - 9.81) < 0.1

def test_two_body_acceleration_direction():
    config = {"use_j2": False}
    gravity = GravityModel(config)  
    r = np.array([6.378137e6, 0, 0]) 
    a = gravity.acceleration(r)
    expected_direction = -r / np.linalg.norm(r) #unit vectore towards earth
    assert np.allclose(a / np.linalg.norm(a), expected_direction)

def test_j2_acceleration_effect():
    config = {"use_j2": True}
    gravity = GravityModel(config)
    r = np.array([6.378137e6, 0, 0])
